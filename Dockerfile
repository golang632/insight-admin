...
RUN apk update && apk add go git musl-dev xz binutils \
    && cd \
    && go get -u -v insight-admin \
    && go install insight-admin
...
RUN wget https://github.com/upx/upx/releases/download/v3.95/upx-3.95-amd64_linux.tar.xz \
    ...
    && cd /root/go/bin \
    && strip --strip-unneeded insight-admin \
    && upx insight-admin \
    && chmod a+x ./insight-admin \
    && cp insight-admin /usr/local/bin

FROM alpine:latest

COPY --from=0 /usr/local/bin/insight-admin /usr/local/bin/
